...
(function () {
  ...
  // Escapes a report-derived string for safe interpolation into HTML
  // templates (element content or double-quoted attributes).
  ...
  function extractRoom(str) {
    if (!str) return null;
    // Capture everything after «مکان:» up to the exam marker, a line break,
    // or end of string. Never use a negated character class of Persian
    // letters here — it would exclude those letters from the room name too.
    const m = str.match(/مکان\s*:\s*([^\n\r]+?)(?=\s+امتحان|امتحان\s*[:：]|\n|$)/);
    if (m && m[1]) {
  ...
  function mergeReports() {
    ...
    eligibleCodes.forEach((code) => {
      ...
      const course = {
        ...from102,
        // Deep-copy sessions so later merges (e.g. replacing report 110) can
        // never mutate the shared catalog102 entries.
        sessions: from102.sessions.map((s) => ({ ...s })),
        exam: from102.exam ? { ...from102.exam } : from102.exam,
        room: from110?.room || null,
        emergencyDrop: from110?.emergencyDrop || null,
        otherCenters: from110?.otherCenters || null,
        notes110: from110?.notes110 || "",
        rawCombined: from110?.rawCombined || "",
      ...
      if (course.sessions.length === 0 && from110?.sessions110?.length > 0) {
        course.sessions = from110.sessions110.map((s) => ({ ...s }));
      }
  ...
  function saveToStorage() {
    try {
      const payload = {
        ...
        pdfDetails: Array.from(state.pdfDetails.entries()),
        filesMeta: {
  ...
  // A stored course is usable only if it has the shape renderAll() relies on:
  // an array of sessions with numeric minute fields and string times.
  function isWellFormedSession(s) {
    return s && typeof s === "object" &&
      DAYS.some((d) => d.key === s.dayKey) &&
      typeof s.startTime === "string" && typeof s.endTime === "string" &&
      Number.isFinite(s.startMin) && Number.isFinite(s.endMin);
  }

  function isWellFormedExam(exam) {
    return exam == null || (typeof exam === "object" && !Array.isArray(exam) &&
      ["date", "startTime", "endTime"].every((key) => exam[key] == null || typeof exam[key] === "string") &&
      ["startMin", "endMin"].every((key) => exam[key] == null || Number.isFinite(exam[key])));
  }

  function isWellFormedCourse(c) {
    return c && typeof c === "object" &&
      typeof c.code === "string" && c.code.length > 0 &&
      typeof c.name === "string" && typeof c.group === "string" &&
      ["credits", "practicalCredits", "capacity", "enrolled", "waitlist"].every((key) => c[key] == null || Number.isFinite(c[key])) &&
      Array.isArray(c.sessions) && c.sessions.every(isWellFormedSession) &&
      isWellFormedExam(c.exam);
  }
  ...
  function loadFromStorage() {
    try {
      ...
      const courses = data && data.mergedCourses;
      if (Array.isArray(courses) && courses.every(isWellFormedCourse)) {
        const reviveMap = (rows, valid) => {
          const map = new Map();
          if (Array.isArray(rows)) {
            rows.forEach((row) => {
              if (!Array.isArray(row) || row.length !== 2) return;
              const [key, value] = row;
              if (typeof key === "string" && valid(value)) map.set(key, value);
            });
          }
          return map;
        };
        state.catalog102 = reviveMap(data.catalog102, isWellFormedCourse);
        state.catalog110 = reviveMap(data.catalog110, (c) => c && typeof c === "object" &&
          Array.isArray(c.sessions110) && c.sessions110.every(isWellFormedSession) && isWellFormedExam(c.exam110));
        state.pdfDetails = reviveMap(data.pdfDetails, (d) => d && typeof d === "object" &&
          Array.isArray(d.prereqs) && d.prereqs.every((p) => p && typeof p === "object") && typeof d.notes === "string");
        state.mergedCourses = courses;
        const courseCodes = new Set(courses.map((c) => c.code));
        state.selectedCodes = new Set(Array.isArray(data.selectedCodes) ? data.selectedCodes.filter((c) => courseCodes.has(c)) : []);
        state.eligibleCodes = new Set(Array.isArray(data.eligibleCodes) ? data.eligibleCodes.filter((c) => typeof c === "string") : courseCodes);
        ["212", "102", "110"].forEach((k) => {
          const meta = data.filesMeta?.[k];
          if (meta && typeof meta.name === "string" && Number.isFinite(meta.size)) {
            state.files[k] = { name: meta.name, size: meta.size };
            updateDropState(k, "is-ok", `بارگذاری‌شده (${meta.name})`);
          }
        });
        return true;
      }
  ...
  function resetStorage() {
    ...
    ["212", "102", "110"].forEach((k) => {
      document.querySelector(`#drop${k} input[type=file]`).value = "";
      updateDropState(k, "", "منتظر فایل");
    });
    ...
    if (chip) {
      if (stateClass === "is-ok") {
        chip.innerHTML = `
          ...
          <span class="drop-chip-text" title="${escapeHtml(fileName || text)}">${escapeHtml(fileName || text)}</span>
          ${countBadge ? `<span class="drop-chip-badge">${escapeHtml(countBadge)}</span>` : ""}
        `;
        ...
        chip.innerHTML = `
          ...
          <span class="drop-chip-text">${escapeHtml(text || "خطا در پردازش فایل")}</span>
        `;
      } else {
        ...
        chip.innerHTML = `
          ...
          <span class="drop-chip-text">${escapeHtml(text || defaultPlaceholders[kind])}</span>
        `;
      }
    }
  }
      ...
      .map((c) => {
        ...
        const sessionsHtml = c.sessions.length
          ? c.sessions
              .map(
                (s) =>
                  `<span><b>${escapeHtml(s.dayName)}:</b> ${escapeHtml(toFa(s.startTime))} تا ${escapeHtml(toFa(s.endTime))} <small>(${escapeHtml(s.typeLabel)})</small></span>`
              )
              .join("")
          : "<span>زمان ارائه نامشخص</span>";

        const examHtml = c.exam && c.exam.date
          ? `<span class="tag tag-exam"><svg viewBox="0 0 24 24" class="ic"><rect x="3" y="5" width="18" height="16" rx="2"/><path d="M8 3v4m8-4v4M3 10h18"/></svg>امتحان: ${escapeHtml(toFa(c.exam.date))}${c.exam.startTime ? ` (${escapeHtml(toFa(c.exam.startTime))})` : ""}</span>`
          : "";

        const roomHtml = c.room
          ? `<span class="tag tag-room"><svg viewBox="0 0 24 24" class="ic"><path d="M20 21V7a2 2 0 0 0-2-2H6a2 2 0 0 0-2 2v14M3 21h18M9 9h1m4 0h1M9 13h1m4 0h1M9 17h1m4 0h1"/></svg>${escapeHtml(toFa(c.room))}</span>`
          : "";

        return `
          <article class="course ${isPicked ? "is-picked" : ""} ${hasConflict ? "is-conflict" : ""}" data-code="${escapeHtml(c.code)}">
            <div class="course-top">
              <div class="course-title">
                <strong class="course-name-clickable" role="button" tabindex="0" data-action="details" data-code="${escapeHtml(c.code)}" title="برای مشاهده جزئیات کلیک کنید">${escapeHtml(c.name)}</strong>
                <small>گروه ${escapeHtml(toFa(c.group))} • کد ${escapeHtml(toFa(c.courseCode))}</small>
              </div>
              <div class="course-actions">
                <button type="button" class="btn btn-sm btn-ghost btn-details" data-action="details" data-code="${escapeHtml(c.code)}" title="مشاهده تمام اطلاعات درس">
                  <svg viewBox="0 0 24 24" class="ic"><circle cx="12" cy="12" r="10"/><path d="M12 16v-4m0-4h.01"/></svg>
                  جزئیات
                </button>
                <button type="button" class="btn btn-sm ${isPicked ? "btn-ghost" : "btn-primary"} course-toggle" data-action="toggle" data-code="${escapeHtml(c.code)}">
                  ${isPicked ? "حذف" : "افزودن"}
                </button>
              </div>
            </div>
            ...
            <div class="course-meta">
              <span class="tag tag-credits">${escapeHtml(toFa(c.credits))} واحد</span>
              <span class="tag tag-prof">${escapeHtml(c.professor)}</span>
              ${roomHtml}
              ${examHtml}
              ${c.capacity ? `<span class="tag">ظرفیت: ${escapeHtml(toFa(c.capacity))}</span>` : ""}
            </div>
  ...
  function clusterDaySessions(daySessions) {
    // Sort by start time, then sweep: extend the open cluster while sessions
    // overlap its running maximum end; otherwise start a new one. This merges
    // clusters bridged by a later session (A 08–09, C 08:30–10:30, B 10–11
    // must all land in one cluster, not [A,C] + [B]).
    const sorted = [...daySessions].sort((a, b) => a.session.startMin - b.session.startMin);
    const clusters = [];
    let current = null;
    let currentEnd = -Infinity;
    for (const item of sorted) {
      const { startMin, endMin } = item.session;
      if (current && startMin < currentEnd) {
        current.push(item);
    ...
    // Restore the caller's original order inside each cluster.
    const originalOrder = new Map(daySessions.map((item, i) => [item, i]));
    clusters.forEach((cl) => cl.sort((a, b) => originalOrder.get(a) - originalOrder.get(b)));
    return clusters;
  }
        ...
        cl.forEach((item, idx) => {
          ...
          colsHtml += `
            <div class="blk-cal ${isConflicted ? "is-conflict" : ""}"
                 ...
                 title="${escapeHtml(`${course.name} (${course.professor}) ${toFa(s.startTime)} تا ${toFa(s.endTime)}${s.room ? ` • ${toFa(s.room)}` : ""}`)}">
              <strong>${escapeHtml(course.name)}</strong>
              <small>${escapeHtml(toFa(s.startTime))}-${escapeHtml(toFa(s.endTime))}${s.room ? ` • ${escapeHtml(toFa(s.room))}` : ""}</small>
            </div>
          `;
        });
      });
  ...
  function renderConflicts() {
    ...
    container.innerHTML = allItems
      .map(
        (c) => `
        <div class="conflict ${c.cssClass}" data-focus="${escapeHtml(`${c.course1.code},${c.course2.code}`)}">
          <span class="kind">
            ${c.icon}
          </span>
  ...
  function openCourseDetails(code) {
    ...
    if (backdrop.hidden) {
      ...
      modalOpenerCode = modalOpener?.dataset.code || null;
      modalOpenerAction = modalOpener?.dataset.action || null;
    ...
    // Header badges
    const headerBadges = [
      `<span class="pill">کد درس: ${escapeHtml(toFa(course.courseCode))}</span>`,
      `<span class="pill">گروه: ${escapeHtml(toFa(course.group))}</span>`,
      `<span class="pill pill-coral">استاد: ${escapeHtml(course.professor)}</span>`,
      course.faculty && isNaN(Number(course.faculty)) ? `<span class="pill">${escapeHtml(course.faculty)}</span>` : "",
      course.dept && isNaN(Number(course.dept)) ? `<span class="pill">${escapeHtml(course.dept)}</span>` : "",
    ].filter(Boolean).join("");
          ...
          ${course.sessions.map((s, i) => `
            <div class="session-item-modal">
              <div style="display: flex; align-items: center; gap: 8px;">
                <strong>جلسه ${toFa(i + 1)}: ${escapeHtml(s.dayName)}</strong>
                <span class="tag tag-credits">${escapeHtml(s.typeLabel)}</span>
              </div>
              <div style="display: flex; gap: 8px; align-items: center;">
                <span class="tag tag-exam">ساعت ${escapeHtml(toFa(s.startTime))} تا ${escapeHtml(toFa(s.endTime))}</span>
                <span class="tag tag-room">${s.room ? `مکان: ${escapeHtml(toFa(s.room))}` : "محل تشکیل: گروه آموزشی"}</span>
              </div>
            </div>
          `).join("")}
        </div>
      ...
      ? `
        <div class="detail-rows">
          <div class="detail-row">
            ...
            <b>${escapeHtml(toFa(course.exam.date))} (تقویم شمسی)</b>
          </div>
          <div class="detail-row">
            <span>ساعت برگزاری آزمون:</span>
            <b>${course.exam.startTime ? `${escapeHtml(toFa(course.exam.startTime))} تا ${escapeHtml(toFa(course.exam.endTime || ""))}` : "ساعت نامشخص"}</b>
          </div>
          <div class="detail-row">
            <span>محل آزمون:</span>
            <b>${course.room ? escapeHtml(toFa(course.room)) : "محل برگزاری متعاقباً اعلام می‌شود"}</b>
          </div>
        </div>
      `
      : `
    ...
    modal.innerHTML = `
      <div class="modal-header">
        <div class="modal-header-text">
          <h2 id="detailsTitle">${escapeHtml(course.name)}</h2>
        ...
        <div class="details-grid">
          ...
          <div class="detail-card">
            ...
            <div class="capacity-visual">
              <div class="capacity-labels">
                <span>ثبت‌نام‌شده: <b>${escapeHtml(toFa(enrolled))} نفر</b></span>
                <span>ظرفیت کل: <b>${escapeHtml(toFa(capacity))} نفر</b></span>
              </div>
            ...
            <div class="mini-stats-grid">
              <div class="mini-stat">
                <small>واحد کل</small>
                <strong>${escapeHtml(toFa(course.credits))}</strong>
              </div>
              <div class="mini-stat">
                <small>واحد عملی</small>
                <strong>${escapeHtml(toFa(course.practicalCredits))}</strong>
              </div>
              <div class="mini-stat">
              ...
              <div class="mini-stat">
                <small>پذیرش</small>
                <strong>${escapeHtml(course.gender || "مختلط")}</strong>
              </div>
            </div>
          </div>
          ...
          <div class="detail-card">
            <h4>
              ...
              قواعد آموزشی و نحوهٔ ارائه درس
            </h4>
            <div class="detail-rows">
              <div class="detail-row">
                <span>نحوهٔ ارائه درس:</span>
                <b>${escapeHtml(course.deliveryMode || "عادی (حضوری)")}</b>
              </div>
              <div class="detail-row">
                <span>دوره / مقطع تحصیلی:</span>
                <b>${escapeHtml(course.coursePeriod || "کارشناسی")}</b>
              </div>
              <div class="detail-row">
                <span>امکان حذف اضطراری (گزارش ۱۱۰):</span>
                <b>${course.emergencyDrop && course.emergencyDrop !== "-" ? escapeHtml(course.emergencyDrop) : "طبق آیین‌نامه دانشگاه"}</b>
              </div>
              <div class="detail-row">
                <span>اخذ توسط سایر مراکز (گزارش ۱۱۰):</span>
                <b>${course.otherCenters && course.otherCenters !== "-" ? escapeHtml(course.otherCenters) : "بررسی با آموزش"}</b>
              </div>
              <div class="detail-row">
                <span>مخصوص ورودی:</span>
                <b>${course.entryFilter && course.entryFilter !== "بی اثر" ? escapeHtml(course.entryFilter) : "همه ورودی‌های مجاز"}</b>
              </div>
              <div class="detail-row">
                <span>دروس همنیاز / متضاد:</span>
                <b>${course.requiredCourses && course.requiredCourses !== "ندارد" ? escapeHtml(course.requiredCourses) : "فاقد درس همنیاز/متضاد"}</b>
              </div>
            </div>
          </div>
          ...
          <div class="detail-card">
            ...
            <div style="font-size: 0.84rem; line-height: 1.85; color: var(--ink); background: var(--paper-2); padding: 12px; border-radius: 10px; border: 1.6px dashed var(--line);">
              ${escapeHtml(course.restrictions || "هیچ محدودیت رشته‌ای یا مقطعی برای این درس ثبت نشده و برای شما مجاز است.")}
            </div>
          </div>
          ...
          ${course.notes || course.notes110 ? `
            <div class="detail-card full-width">
              ...
              <div style="font-size: 0.85rem; line-height: 1.8; color: var(--ink);">
                ${course.notes ? `<div><b>توضیحات گزارش ۱۰۲:</b> ${escapeHtml(course.notes)}</div>` : ""}
                ${course.notes110 ? `<div><b>توضیحات گزارش ۱۱۰:</b> ${escapeHtml(course.notes110)}</div>` : ""}
              </div>
            </div>
          ` : ""}
          ...
          <div class="detail-card full-width">
            ...
            <div style="font-family: monospace; font-size: 0.76rem; background: var(--paper-2); padding: 12px; border-radius: 10px; border: 1.6px dashed var(--line); display: flex; flex-direction: column; gap: 6px;">
              <div><b>زمان و مکان ارائه (۱۰۲):</b> ${escapeHtml(course.rawSchedule || "—")}</div>
              <div><b>زمان و مکان امتحان (۱۰۲):</b> ${escapeHtml(course.rawExam || "—")}</div>
              ${course.rawCombined ? `<div><b>زمان، مکان و امتحان تلفیقی (۱۱۰):</b> ${escapeHtml(course.rawCombined)}</div>` : ""}
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="modal-footer">
        <button type="button" class="btn ${isPicked ? "btn-ghost" : "btn-primary"}" data-action="toggle-from-modal" data-code="${escapeHtml(course.code)}">
  ...
  function closeCourseDetails() {
    ...
    let target = modalOpener;
    if (!target?.isConnected && modalOpenerCode) {
      target = Array.from(document.querySelectorAll("[data-code]")).find((el) =>
        el.dataset.code === modalOpenerCode && el.dataset.action === modalOpenerAction && el.tabIndex >= 0);
    }
        ...
        cl.forEach((item, idx) => {
          ...
          blocks += `
            <div class="wblk${conflicted ? " is-conflict" : ""}"
              ...
              <strong>${escapeHtml(course.name)}</strong>
              <span>${escapeHtml(toFa(s.startTime))}–${escapeHtml(toFa(s.endTime))}${s.room ? ` • ${escapeHtml(toFa(s.room))}` : ""}</span>
            </div>`;
        });
      });
  ...
  function exportTimetable() {
    ...
    const rowsHtml = selected
      .map((c, i) => `
        <tr>
          <td>${toFa(i + 1)}</td>
          <td>${escapeHtml(toFa(c.courseCode))}</td>
          <td>${escapeHtml(toFa(c.group))}</td>
          <td class="c-name">${escapeHtml(c.name)}</td>
          <td>${escapeHtml(toFa(c.credits))}</td>
          <td>${escapeHtml(c.professor)}</td>
          <td>${c.room ? escapeHtml(toFa(c.room)) : "—"}</td>
          <td class="c-sessions">${c.sessions.map((s) => `${escapeHtml(s.dayName)} ${escapeHtml(toFa(s.startTime))}–${escapeHtml(toFa(s.endTime))}`).join("<br>") || "—"}</td>
          <td>${c.exam && c.exam.date ? `${escapeHtml(toFa(c.exam.date))}<br><small>${escapeHtml(toFa(c.exam.startTime || ""))}${c.exam.endTime ? `–${escapeHtml(toFa(c.exam.endTime))}` : ""}</small>` : "—"}</td>
        </tr>
      `)
      .join("");
...
.wgrid { display:grid; grid-template-columns:54px repeat(${DAYS.length},1fr); border:2px solid var(--ink); border-radius:12px; overflow:hidden; }
...
<body>
  ...
  function setupUploadHandlers() {
    ["212", "102", "110"].forEach((kind) => {
      ...
      input.addEventListener("change", () => processUpload(input.files[0]));
  ...
  function setupInteractions() {
    ...
    document.getElementById("courseList").addEventListener("keydown", (e) => {
      const trigger = e.target.closest(".course-name-clickable");
      if (trigger && (e.key === "Enter" || e.key === " ")) {
        e.preventDefault();
        openCourseDetails(trigger.dataset.code);
      }
    });
    ...
    document.getElementById("conflictList").addEventListener("click", (e) => {
      ...
      codes.forEach((c) => {
        const el = Array.from(document.querySelectorAll(".course[data-code]")).find((course) => course.dataset.code === c);
        if (el) {
    ...
    if (mTabs && plannerGrid) {
      ...
      mTabs.addEventListener("click", (e) => {
        ...
        mTabs.querySelectorAll(".m-tab").forEach((b) => {
    ...
    // Contain keyboard and programmatic focus while the details dialog is open.
    document.addEventListener("keydown", (e) => {
      ...
      if (e.key === "Escape") {
        ...
        const buttons = Array.from(modal.querySelectorAll("button:not([disabled]), [tabindex='0']"));
    ...
    document.addEventListener("focusin", (e) => {
      if (backdrop && !backdrop.hidden && !backdrop.contains(e.target)) {
        document.querySelector("#detailsModal [data-action='close-modal']")?.focus();
      }
    });
